...
public class flob_collide extends PApplet {
...
public void draw(){
  ...
  for(int i=0;i<bolas.length;i++){
    ...
    if(cdata[0] > 0) {
      bolas[i].toca=true;
      bolas[i].vx +=cdata[1]*width*0.015f;
      bolas[i].vy +=cdata[2]*height*0.015f;
    } 
    else {
      bolas[i].toca=false; 
    }
...
class Bola{
  float x,y,vx,vy;
  float g = 0.025f, rad= random(5,25);
  boolean toca=false;
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#FFFFFF", "flob_collide" });
  }
}
